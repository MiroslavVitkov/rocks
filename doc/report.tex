...
random forest  & 0.17                    & \SI{21}{\second} & \SI{1}{\hour} \SI{49}{\minute}   & multithreaded \\  % doesn't work! evil andres::trees
\end{tabular}
...
However, the used implementation allows the use only linar kernels for multiclass SVMs (why)?
A grid search for the regularization coefficient c in the range [1e-5, 1e5] indicates said space is rather flat.
Also notice the drop in accuracy compated to the results reported in chapter 'Model'.
This is due to training on a smaller dataset, due to witholding the validation partition.
\begin{tabular}{ c | c }
c & accuracy  \\
1e-5 & 0.788652  \\
1e-4 & 0.780142  \\
